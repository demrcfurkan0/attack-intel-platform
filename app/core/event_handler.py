...
async def startup_event():
    ...
    current_script_directory = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../"))
    
    # Model yollarını ana dizine göre birleştir
    actual_model_path = os.path.join(current_script_directory, Config.MODEL_PATH)
    actual_scaler_path = os.path.join(current_script_directory, Config.SCALER_PATH)
    actual_feature_columns_path = os.path.join(current_script_directory, Config.FEATURE_COLUMNS_PATH)
    ...
    try:
        ...
        if feature_columns:
            ...
            print(" Özellik sütunları yüklenemedi veya boş.")
